...
class Triangle2D {
  ...
  boolean contains(Point2D p) {
    ...
    if((abn & side) != (bcn & side)) return false;  // signs (aka sides) don't match
    if((abn & side) != (can & side)) return false;  // signs (aka sides) don't match
    // in
    return true;
  }
  ...
  Point2D findCircumCenter() {
    ...
    { 
        ...
        // by returning a pair of MAX_FLOAT 
  ...
  boolean isPointInCircumCircle(final Point2D p) {
    float a11 = a.x - p.x;
    float a21 = b.x - p.x;
    float a31 = c.x - p.x;
  ...
  Point2D getNonEdgeVertex(final Edge2D e0) {
  ...
  EdgeDistancePack findNearestEdge(final Point2D p) {
   ...
   */
  Point2D findClosestPoint(final Edge2D edge, final Point2D p) {
    float abx = edge.b.x-edge.a.x;
    float aby = edge.b.y-edge.a.y;
    float pax = p.x-edge.a.x;
    float pay = p.y-edge.a.y; 
    
    float t = ( pax*abx + pay*aby ) / (abx*abx + aby*aby);
    if(t<0) t=0;
    else if(t>1) t=1;
    
    float px=edge.a.x+abx*t;
    float py=edge.a.y+aby*t;
    stroke(255,255,0);
    point(px,py);
